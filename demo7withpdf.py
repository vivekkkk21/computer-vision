...
IGNORE_CLASSES = {"person", "car", "potted plant", "Microwave", "chair"}
STANDARD_LIST = {"laptop", "desktop", "keyboard", "mouse", "files", "pages", "paper tray",
                 "calendar", "headphone", "mobile phone", "nameplate", "book", "document", "stickynote"}
    ...
    def _process_thread(self):
        try:
            ...
            for d in detections:
                ...
                cv2.rectangle(annot, (x1, y1), (x2, y2), color, 10)
                cv2.putText(annot, f"{lbl} {conf:.2f}", (x1, y1 - 8),
                            cv2.FONT_HERSHEY_SIMPLEX, 8, color, 12)
    ...
    def save_report(self):
        ...
        try:
            ...
            if self.annotated_image and os.path.exists(self.annotated_image):
                ...
                c.drawImage(self.annotated_image, 1 * inch, 2 * inch,
